...
- Before contributing, you should consider [opening an issue in Launchpad](https://bugs.launchpad.net/snap-openstack-hypervisor) explaining your use case.
...
For testing a local change, you may wish to build and deploy the snap yourself. Install the locally built snap in dangerous mode (required because it is an unsigned package). However, snap interfaces installed in dangerous mode will not be automatically connected. The easiest way to connect the snap is to first install from the snap store, then install the local snap in dangerous mode. This will ensure that all the required interfaces are connected.

```bash
sudo snap install openstack-hypervisor --channel <existing-channel>
...
After applying changes, restart the snap:
