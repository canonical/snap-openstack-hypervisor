    ...
    def __init__(self, pci_addr: PCISpecAddressType) -> None:
        try:
            ...
            if isinstance(pci_addr, dict):
                ...
                self.domain, self.bus, self.slot, self.func = get_pci_address_fields(pci_addr)
...
class WhitelistPciAddress(object):
    ...
    def __init__(self, pci_addr: PCISpecAddressType, is_physical_function: bool) -> None:
...
class PciDeviceSpec(PciAddressSpec):
    ...
    def _init_dev_details(self) -> None:
        ...
        if address and self.dev_name:
            raise Exception("The PCI whitelist can specify devname or address," " but not both")
